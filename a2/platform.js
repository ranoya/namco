...
if (!__platform_js__) {
  ...
  function PlatformSpecific() {
    ...
    if (this.op && !this.op6)
      document.onmousedown = function (e) {
        if (((e = e || window.event).target || e.srcElement).tagName == "IMAGE")
          return false;
      };
    ...
    if (
      (this.webkit =
        this.ua.match(/AppleWebKit\/(\d+)/i) ||
        undefined) /* match returns null which sucks for inequalities */
    )
    ...
    this.has_position_fixed = (function () {
      // Adapted from http://yura.thinkweb2.com/cft/
    ...
    this.key_name = {
      ...
      8: "backspace",
      27: "escape",
      6: "delete" /*mac firefox*/,
      // Explorer
      18: "" /*alt(will have A- added below)*/,
      16: "S-" /*shift*/,
      192: "`",
      189: "-",
      187: "=",
    ...
    this.explorer_shift_map = {
      // These shifts of the above sequence are so specific to *my* keyboard it's not even funny.
      192: "~",
      189: "_",
      187: "+",
      219: "{",
  ...
  PlatformSpecific.prototype.windowWidth = function () {
    return document.documentElement.clientWidth;
  };
  PlatformSpecific.prototype.windowHeight = function () {
  ...
  PlatformSpecific.prototype.register_key_events = function (div, handler) {
    ...
    for (var a in args) handlers[args[a]] = 1;
    ...
    if (handlers.keydown) div.addEventListener("keydown", down, true);
    if (handlers.keypress) div.addEventListener("keypress", down, true);
    ...
    return function () {
      if (handlers.keydown) div.removeEventListener("keydown", down, true);
      if (handlers.keypress) div.removeEventListener("keypress", down, true);
  ...
  plat.key_event = function (event) {
    ...
    if (event.keyIdentifier) {
      // DOM 3 (Safari 4, for starters)
      var m;
      if (event.keyIdentifier.toLowerCase() == "u+0020") this.key = "space";
      else if (event.keyIdentifier.toLowerCase() == "u+0009") this.key = "tab";
      else if (event.keyIdentifier.toLowerCase() == "u+0008")
        this.key = "backspace";
      else if (event.keyIdentifier.toLowerCase() == "u+001b")
        this.key = "escape";
      else if (event.keyIdentifier.toLowerCase() == "enter")
        this.key = "return";
      else if ((m = event.keyIdentifier.match(/^U\+([\da-f]{4})$/i)))
        event.shiftKey
          ? (this.key = chr(parseInt("0x" + m[1])).toUpperCase())
    ...
    else if (charCode < 32 && event.ctrlKey)
      this.key = chr(charCode + 96); // Safari gives ascii control chars
    else if (charCode == 32) this.key = "space";
    else if (this.shiftKey && plat.explorer_shift_map[this.keyCode])
      this.key = plat.explorer_shift_map[this.keyCode];
    else if (plat.dom_key_code(event) != undefined)
      this.key = plat.dom_key_code(event);
    else if (plat.key_name[keyCode] != undefined)
      this.key = plat.key_name[keyCode];
    else {
      if (isupper(keyCode) && !this.shiftKey)
        // Stupid explorer!
        keyCode = tolower(keyCode);
      if (isgraph(keyCode))
        this.key = chr(keyCode); // IE puts things in the keycode
      else if (keyCode == 32) this.key = "space"; // Explorer
      else this.key = "\\" + (keyCode || charCode);
    }
    if (this.shiftKey)
      this.key =
        this.key.length == 1 ? this.key.toUpperCase() : "S-" + this.key;
...
*/
      ...
      if (typeof component == "object") {
        for (var p in (param = component))
          if (p != undefined && param[p] != undefined)
            params.push(
              encodeURIComponent(p) + "=" + encodeURIComponent(param[p])
            );
      }
    }
    return params.join("&");
  ...
  function makeurl() {
    ...
    for (var i = 0; i < arguments.length; i++) {
      var component = arguments[i],
        param;
      if (typeof component == "object") params.push(component);
      else if (component != undefined)
        if (protocol == "" && !url.length && component.match(/:$/))
          protocol = component;
        else url.push(encodeURIComponent(component));
    }
    return (
      protocol +
      url.join("/") +
      (params.length == 0 ? "" : "?" + makeurlparams.apply(null, params))
    );
  }
    ...
    if (req) {
      req.onreadystatechange = function () {
        if (f && req.readyState == 4) {
          ...
          try {
            // we access the req function here to centralize the try/catch
            var text = req[method || "responseText"],
              status = req.status,
            ...
            text = undefined;
            error = err || "unknown request error";
          }
          if (status != 200 && !url.match(/^file:/)) {
            // this used to be == 404 which seems a bit too specific
            text = undefined;
            error = statusText || "unknown status text";
          }
          //req.onreadystatechange = null;
          if (f) f(text, error, status);
        }
      };
      if (document.URL.match(/^file:\/\//)) {
        // relative URLs don't seem to work (in Chrome, at least)
        var m,
      ...
      try {
        ...
        if (f) f(undefined, "description" in e ? e.description : "" + e, -1);
      }
  ...
  function loadBinary(url, f) {
    ...
    return loadContent(
      ...
      null,
      ff,
      plat.ie ? "responseBody" : "responseText",
      true
    );
  }
  ...
  function _interpJSON(f, text, error, status) {
    ...
    if (json && json.status != "ok") {
      // allow the json to return an error in a consistent way
      error = json.status;
      json = undefined;
    }
  ...
  function postForm(url, data, f) {
    ...
    if (plat.ua == "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)")
      return loadContent(
        url + (url.match(/\?/) ? "&" : "?") + makeurlparams(data),
        null,
        f
      );
    return loadContent(
  ...
  function postJSON(url, data, f) {
    return loadContent(
      url,
      { data: json(data), mimetype: "application/json; charset=UTF-8" },
      f
    );
  }
  ...
  var debug = 1;
  on_load(function () {
    if (URL.param.debug != undefined) debug = URL.param.debug;
  });
  function report_error(where, error, extra) {
    if (debug) {
      ...
      postJSON(
        makeurl("error", "report"),
        merge(
          { Message: "Caught in " + where + ": " + error, e: error },
          extra ? { extra: extra } : {}
        )
      );
  }
  ...
  function print(s) {
    var flush;
    if (print_array.push(s) == 1)
      // cache up requests since innerHTML is slow
      timeout(
        0,
        (flush = function () {
      ...
      case "object":
        ...
        if ("nodeType" in s) rep += "DOM(" + html_from_dom(s) + ")";
          ...
          rep += wrap(
            "[",
            map(function (v) {
              return json(v, undefined, newindent);
            }, s),
            "]"
          );
        else
          rep += wrap(
            "{",
            map(function (v, k) {
              return '"' + k + '": ' + json(v, undefined, newindent);
            }, s),
            "}"
          );
        break;
    ...
    if (!s) print(" " + name + " is undefined\n");
      ...
      for (var i in s) {
        ...
        try {
          ...
          rep = "!" + (typeof err == "string" ? err : typeof err) + "!";
        }
        print(" " + name + "." + i + "=" + rep + "\n");
      }
    print("--------------\n");
  ...
  function sprintf(format) {
    ...
    var chunk = f.split(/%/);
    var out = "";
    if (f.slice(0, 1) != "%" || chunk[0] == "") {
      // Skip the next steps on IE since it does split *WRONG*!!! Bastards.
    ...
    for (var c in chunk) {
      ...
      if (m == undefined) {
        out += "[Bad printf sequence: %" + chunk[c] + "]";
        continue;
      }
      if (m[3] == "") m[3] = 0; // 3. => 3.0
      var width = parseInt(m[2] == "*" ? arguments[a++] : m[2]),
        ...
        flags = {};
      for (var i = 0; i < m[1].length; i++) flags[m[1].slice(i, 1)] = 1;
      if (m) {
        ...
        switch (m[4]) {
          case "q":
            ...
            break;
          case "d":
            arg += pad(parseInt(arguments[a++]), "0", prec);
            break;
          case "s":
            arg += arguments[a++];
            if (prec) arg = arg.slice(0, prec);
            break;
          case "f":
            arg += parseFloat(arguments[a++]).toFixed(
              prec == undefined ? 10 : prec
            );
            break;
          case "x":
            arg += pad(parseInt(arguments[a++]).toString(16), "0", prec);
            break;
          case "j":
            arg += json(arguments[a++]);
            break;
          case "c":
            arg +=
              typeof arguments[a] == "string"
                ? arguments[a++].slice(0, 1)
                : String.fromCharCode(arguments[a++]);
            break;
        }
  ...
  function say(hash) {
    s = [];
    for (var v in hash) s.push(v + "=" + hash[v]);
    print(s.join(", ") + "\n");
  }
  ...
  function new_alert(dom_el, options) {
        ...
        [
          ...
          [
            ...
            [
              "td",
              { className: "ctm" },
              ["div", { className: "alert", style: { width: px(w) } }, dom_el],
            ],
    ...
    div.unregister_escape = plat.register_key_events(
      document,
      wrap(
        this,
        function (e) {
    ...
    div.style.top = px(
      Math.max(0, plat.windowHeight() / 2 - div.clientHeight / 2)
    );
    return div;
  }
  ...
  function close_alert(alert) {
    ...
    if (alert.unregister_escape) alert.unregister_escape();
    try {
      document.body.removeChild(alert);
    } catch (e) {
      /* Could be closed already. We don't care. */
    }
  }
  ...
  CookieClass.prototype.reload = function () {
    ...
    for (var c in cookie)
      this.list[decodeURIComponent(cookie[c].split("=")[0])] =
        decodeURIComponent(cookie[c].split("=")[1]);
  };
  CookieClass.prototype.set = function (
    key,
    value,
    max_age,
    path,
    ...
    /* Delete */ else cookie += ";expires=Sat, 19 Jan 1974 04:31:24 GMT";
  ...
  var doc = function (array) {
    ...
    if (array.constructor === String) return document.createTextNode(array);
    var el = array[0];
    ...
    for (var i = 1, l = array.length; i < l; i++) {
      var a = array[i];
      if (!valid(a, array)) continue;
      if (a.constructor === Array) {
        if (a.length === 0 || a[0].constructor === Array)
          for (var j in a) // [[],[],...] ==> [],[],...
            el.appendChild(doc(a[j]));
        else el.appendChild(doc(a));
      } else if (typeof a.nodeType !== "undefined") el.appendChild(a);
      else if (a.constructor === Object)
        for (var p in a) {
          var ap = a[p];
          if (valid(ap, a) && ap.constructor === Object)
            for (s in ap) el[p][s] = ap[s];
          else el[p] = ap;
        }
      else el.appendChild(document.createTextNode(a));
    }
    return el;
  };
  (doc.el = function (elem, props) {
    ...
    if (props)
      for (var p in props)
        ...
        else for (var s in props.style) el.style[s] = props.style[s];

    if (arguments.length > 2) doc.append_array(el, arguments, 2);
    return el;
  }),
    ...
    (doc.append_array = function (dom_el, array, start) {
      for (var a = start || 0; a < array.length; a++)
        if (array[a])
          ...
          else dom_el.appendChild(array[a]);
    }),
    ...
    (doc.opacity = function (el, percent) {
      ...
      if (plat.ie < 9) {
        if (plat.ie <= 6) return; // below filter conflicts with cards AlphaImageLoader filter.
        el.style["-ms-filter"] =
  ...
  doc.click = function (el) {
    if (el.nodeName == "A")
      // Firefox doesn't follow <a> links on synthetic click events :-(
      location = el.href;
    else if (document.createEvent) {
      // Firefox, DOM Level 2,3 Events:
  ...
  function isspace(c) {
    return c == 9 || c == 10 || c == 11 || c == 13 || c == 28;
  }
  function isalpha(c) {
  ...
  function filter(f, a) {
    var b = [];
    for (var i in a) if (f(a[i])) b.push(a[i]);
    return b;
  }
  function filter_obj(f, a) {
    var b = [];
    for (var i in a) if (f(a[i], i)) b[i] = a[i];
    return b;
  }
  function map(f, a) {
    var b = [];
    for (var i in a) b.push(f(a[i], i));
    return b;
  }
  ...
  function arrayize(a) {
    // For stupid DOM things that look like arrays but don't act like them.
    var b = [];
  ...
  function rot13(s) {
    ...
    for (var c in sa) {
      var base =
        sa[c].toLowerCase() == sa[c] ? "a".charCodeAt(0) : "A".charCodeAt(0);
      out +=
        sa[c].toLowerCase() < "a" || sa[c].toLowerCase > "z"
          ? sa[c]
          : String.fromCharCode(
              ((sa[c].charCodeAt(0) - base + 13) % 26) + base
            );
    }
    return out;
  }
